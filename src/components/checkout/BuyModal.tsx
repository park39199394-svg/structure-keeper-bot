...
const BuyModal = ({ open, onClose }: BuyModalProps) => {
  ...
  const selected = colorOptions.find((c) => c.id === selectedColorId) ?? colorOptions[0];
    ...
    <div className="fixed inset-0 z-[60] flex items-end justify-center">
      ...
      <div className="relative w-full max-w-lg bg-background rounded-t-2xl animate-in slide-in-from-bottom duration-300 max-h-[85vh] overflow-y-auto">
          ...
          <div className="p-4 space-y-4">
            <div>
              <h3 className="text-sm font-semibold text-foreground mb-3">
                {colorOptions.length > 1 ? `Cor (${colorOptions.length})` : "Opção"}
              </h3>
                ...
                {colorOptions.map((color) => (
                  ...
                  >
                    <div className="relative w-full aspect-square bg-muted">
                      <img src={color.image} alt={color.label} className="w-full h-full object-contain p-2" />
                      {selectedColorId === color.id && (
                    ...
                    <span className="text-xs font-medium text-foreground pb-2">{color.label}</span>
                  </button>
                ))}
              </div>
            </div>
        ...
        <div className="sticky bottom-0 bg-background border-t border-border p-4">
          <button
            onClick={() => {
              if (selected?.checkoutUrl) window.open(selected.checkoutUrl, "_blank");
            }}
