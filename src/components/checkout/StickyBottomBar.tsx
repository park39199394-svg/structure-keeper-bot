...
const StickyBottomBar = ({ onBuyClick }: StickyBottomBarProps) => {
  return (
    <div className="fixed bottom-0 left-0 right-0 bg-background border-t border-border flex items-center px-2 py-2 gap-1.5 z-50">
      ...
      <button
        onClick={onBuyClick}
        className="flex-1 bg-primary text-primary-foreground py-2.5 rounded-full text-xs font-bold hover:opacity-90 transition-opacity whitespace-nowrap"
      >
        COMPRAR AGORA
      </button>
    </div>
